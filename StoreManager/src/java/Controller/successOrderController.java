...
import static Controller.orderController.number;
import static Controller.orderController.page;
...
static int page = 0;
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet successOrderController</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet successOrderController at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
       ...
       HttpSession sessionList = request.getSession();
        processOrderDAO processDao= new processOrderDAO();
            ...
            String pageNumber =request.getParameter("myBTN");
             ...
             try {
                 if (sessionList.getAttribute("flagAddSuccessGuest")==null) {
                 listprocessOrder = processDao.getAllSuccessOrder();}
            ...
            if (!listprocessOrder.isEmpty()) {
				...
				sessionList.setAttribute("setStatus", 0);
				 ...
				 request.getRequestDispatcher("Views/Product/SuccessOrders.jsp").forward(request, response);
	           }
             else {
					page=0;
					number=0;
      ...
      HttpSession sessionList = request.getSession();
        ...
        if (submitSearch != null) {
           ...
           processOrderDAO proDao = new processOrderDAO();
            ...
            try {
                  listprocessOrder = proDao.getAllSuccessOrderBySearch(searchString);
                  sessionList.setAttribute("flagAddSuccessGuest", 0);
