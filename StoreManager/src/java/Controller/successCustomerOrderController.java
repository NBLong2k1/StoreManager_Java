 ...
 * @author notur
 */
public class successCustomerOrderController extends HttpServlet {
static int page = 0;
    static int number = 0;
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet successCustomerOrderController</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet successCustomerOrderController at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
       ...
       HttpSession sessionList = request.getSession();
        customerProcessOrdersDAO processDao= new customerProcessOrdersDAO();
            ...
            String pageNumber =request.getParameter("myBTN");
             ...
             try {
                  if (sessionList.getAttribute("flagSuccessCustomer")==null) {
            ...
            if (!listprocessOrder.isEmpty()) {
				...
				sessionList.setAttribute("setStatus", 0);
				 ...
				 request.getRequestDispatcher("Views/Product/SuccessCustomerOrder.jsp").forward(request, response);
	           }
             else {
					page=0;
					number=0;
      ...
      HttpSession sessionList = request.getSession();
        ...
        if (submitSearch != null) {
           ...
           customerProcessOrdersDAO proDao = new customerProcessOrdersDAO();
            ...
            try {
                  listprocessOrder = proDao.getAllSuccessOrderByID(searchString);
                  sessionList.setAttribute("flagSuccessCustomer", 0);
