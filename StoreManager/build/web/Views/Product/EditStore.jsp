    ...
    <body id="page-top">
        ...
        <div id="wrapper">
            ...
            <ul class="navbar-nav bg-gradient-primary sidebar sidebar-dark accordion" id="accordionSidebar">
                ...
                <!-- Sidebar - Brand -->
               <c:if test="${role==1}">
                    <a class="sidebar-brand d-flex align-items-center justify-content-center" href="<%= request.getContextPath()%>/Views/Account/Admin.jsp">

                    </c:if>
                    <c:if test="${role==0}">
                        <a class="sidebar-brand d-flex align-items-center justify-content-center" href="<%= request.getContextPath()%>/Views/Account/EmployeeManager.jsp">

                        </c:if>  <div class="sidebar-brand-icon rotate-n-15">
                        <i class="fas fa-laugh-wink"></i>
                    </div>



                    <div class="sidebar-brand-text mx-3">Employee ${username}</div>
                ...
                <li class="nav-item active">
                    <c:if test="${role==1}">
                    <a class="nav-link" href="<%= request.getContextPath()%>/Views/Account/Admin.jsp">

                    </c:if>
                    <c:if test="${role==0}">
                        <a class="nav-link" href="<%= request.getContextPath()%>/Views/Account/EmployeeManager.jsp">
                    ...
                    <div class="container-fluid" id="EmployeeInsert" style="display: block" >
                        ...
                        <div class="row">
                            <div class="col-7"> 
                                ...
                                <form action="<%= request.getContextPath()%>/EditStoreController" method="post">
                                    <div class="mb-3">
                                        <label for="StoreName" class="form-label">Tên Cửa Hàng</label>
                                        <input type="text" class="form-control" value="${store.getStore_name()}" name="StoreName" required placeholder="Nhập Tên Cửa Hàng">
<input type="text" class="form-control" value="${store.getStore_id()}" hidden name="StoreID" >

                                    </div>
                                    <div class="mb-3">
                                        <label for="productDescription" class="form-label">Địa Chỉ</label>
                                        <input type="text" name="StoreAddress" value="${store.getStore_address()}" required placeholder="Nhập Địa Chỉ" class="form-control" >

                                    </div>
                                    <div class="mb-3">
                                        <label for="productPrice" class="form-label">Thành Phố</label>
                                        <input type="text" name="StoreCity" value="${store.getCity()}" required placeholder="Nhập Thành Phố" class="form-control" >
                                    </div>

                                    <div class="mb-3">
                                        <label for="productImage" class="form-label">Số Điện Thoại</label>
                                        <input type="number" name="StorePhone" value="${store.getPhone()}" required placeholder="Nhập Số Điện Thoại" class="form-control" >

                                    </div>


                                    </br>
                                    <button type="submit" name="submitEditStore" class="btn btn-primary">Lưu Thông Tin</button> 
                                    <a href="<%= request.getContextPath()%>/StoreController" class="btn btn-success">Trở Lại</a>
